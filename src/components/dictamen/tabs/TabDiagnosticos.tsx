...
export default function TabDiagnosticos({
  dictamenId,
  procedimientoPcl,
  cie10Options, // no se usa ahora, lo dejamos por compatibilidad
  initialDiagnosticos,
}: Props) {
  const {
    diagnosticos,
  ...
  const [primaryError, setPrimaryError] = useState(false); // ⬅️ error en diagnóstico principal
  ...
  const handleSaveRemote = async () => {
    ...
    if (payload.length === 0) {
      // Esto casi nunca pasará porque ya validamos el primero,

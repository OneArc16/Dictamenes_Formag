import { scheduleAppointmentInTransaction } from '@/features/reception/application/reception-service';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const operation = await executeIdempotent({ actorEmpleadoId: auth.auth.empleadoId, operationType: 'APPOINTMENT_SCHEDULE', idempotencyKey: idempotencyKey(request), payload: input, execute: (tx) => scheduleAppointmentInTransaction(tx, auth.auth, input) });

...
function ScheduleDateItem({
  ...
  return (
    ...
    >
          ...
          {canToggle ? (
            <IconTooltip id={tooltipId} label={actionLabel} align="right">
              <Button
                ...
                className={cn(
                  'h-11 w-11 rounded-full transition-colors duration-200',
                  doctorExcluded
                    ? 'text-sky-700 hover:bg-sky-100 hover:text-sky-800'
                    : 'text-rose-700 hover:bg-rose-100 hover:text-rose-800',
                )}
                onClick={() =>
                  onExcludedDateChange?.(schedule.date, !doctorExcluded)
                }
              >
                {doctorExcluded ? (
      ...
      <div className="mt-1.5 flex items-start gap-1.5 text-xs leading-5">
        <Clock3
          className={cn(
...
export function DoctorScheduleSummary({
  ...
  return (
    <section
      ...
      className="min-w-0 rounded-xl border border-slate-100 bg-slate-50/40 p-3"
    >

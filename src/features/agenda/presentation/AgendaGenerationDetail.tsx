...
export default function AgendaGenerationDetail({ id, canCancel }: { id: number; canCancel: boolean }) {
  ...
  const cancellation = useMutation<{ cancelados: number; omitidos: number }, Error>({
    mutationFn: () => agendaRequest('/api/agenda/cupos/cancelar', {
      method: 'POST', body: JSON.stringify({ cupoIds: [...selected], motivo: 'Cancelación desde el detalle de generación' }),
    }),
